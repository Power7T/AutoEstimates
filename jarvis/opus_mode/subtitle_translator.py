...
2. Parse every ``Dialogue:`` line and extract visible text (tags stripped).
3. Translate visible text in batches of 50 lines via the AI router.
4. Inject translated text back, preserving all ASS override tags
...
-----
    ...
    print(f"Translated file written to: {out}")
"""
...
#   Group 2 — the raw text field (visible words + override tags)
_DIALOGUE_RE = re.compile(
    r"^(Dialogue:[^\n]*,,)"   # prefix up to and including last ",,"
    r"(.*)$",                  # raw text field
    re.MULTILINE,
)
...
# Lines per AI translation request
_BATCH_SIZE = 50
...
def translate_subtitles(
    ...
    output_path:
        ...
        ``<original_stem>.<lang_slug>.ass`` in the same directory.
    ...
    # --- 2. Translate in batches of _BATCH_SIZE ------------------------------
    ...
    for batch_start in range(0, len(all_texts), _BATCH_SIZE):
        ...
        logger.debug(
            "Translated batch %d–%d.",
            batch_start,
            batch_start + len(batch_texts) - 1,
        )

    # --- 3. Reassemble ASS with translated text --------------------------------
...
def _extract_dialogue_text(ass_content: str) -> list[tuple[int, str]]:
    ...
    *match_index* is the zero-based index of the regex match in the file —
    it is used later to map translations back to the correct line.

    *visible_text* has all ``{...}`` override tags stripped and is trimmed of
    leading/trailing whitespace. Lines that are empty after stripping are
    excluded (they typically contain only positioning tags).
    """
...
def _translate_batch(texts: list[str], target_language: str) -> list[str]:
    ...
    # Map index → original text for the JSON payload
    ...
    system_prompt = (
        ...
        "Rules:\n"
        "- Preserve the meaning, tone, and natural spoken feel.\n"
        "- Keep translations concise — subtitles must be read quickly.\n"
    ...
    user_prompt = (
        ...
        "Return only a valid JSON object with the translated values. "
    ...
    try:
        ...
        # Rebuild list in original order, falling back to source on missing keys
...
# ---------------------------------------------------------------------------
# Step 3 — Reassemble with tags preserved
# ---------------------------------------------------------------------------
...
def _reassemble_ass(
    ...
    with the corresponding translation while keeping all override tags intact.

    Processes matches in reverse order so that string offsets stay valid.
    """
    ...
    for match_idx in range(len(matches) - 1, -1, -1):
        ...
        raw_text         = m.group(2)        # original text field (tags + words)
...
def _inject_tags(original_raw: str, translated_visible: str) -> str:
    ...
    2. Count how many words belong to each text token proportionally.
    3. Distribute the words of *translated_visible* across those same slots,
       keeping the same proportional word counts.
    4. Re-interleave tags and translated text chunks.

    This faithfully preserves ``{\\k}`` karaoke timing, ``{\\an}`` alignment,
    and other positional/style tags. Edge cases (empty text, tags-only lines)
    ...
    for i, src_count in enumerate(word_counts):
        ...
        # Preserve the leading/trailing whitespace pattern of the original chunk
        orig            = text_chunks[i]
